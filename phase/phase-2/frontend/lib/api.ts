...
const NEON_API_BASE_URL = (process.env.NEXT_PUBLIC_NEON_API_URL || 'http://localhost:8000/api/v1').replace(/\/$/, '');

// Determine which API base URL to use
const getBaseUrl = (): string => {
  if (process.env.NEXT_PUBLIC_USE_NEON_API === 'true') {
    return NEON_API_BASE_URL;
  }
  return API_BASE_URL;
};
